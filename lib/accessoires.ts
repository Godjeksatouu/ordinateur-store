...
export async function fetchAccessoires(): Promise<Accessoire[]> {
  try {
    console.log('Fetching accessoires from:', `${API_BASE_URL}/api/accessoires`);
    const response = await fetch(`${API_BASE_URL}/api/accessoires`);

    console.log('Accessoires response status:', response.status);

    if (!response.ok) {
      const errorText = await response.text();
      console.error('Accessoires fetch error:', response.status, errorText);
      throw new Error(`Failed to fetch accessoires: ${response.status} ${response.statusText}`);
    }

    const data = await response.json();
    console.log('Accessoires data received:', data?.length || 0, 'items');
